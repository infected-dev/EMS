...
from .cust_functions import convert, minutes, timeobj, session_date
...
def mill_vehicles():
    if request.form:
        ...
        db.session.commit()
        session['saved_date_mill'] = entrydate
...
def post_vehicles():
    today_date = ''
    if 'saved_date' in session:
        today_date = session['saved_date']
        today_date = session_date(today_date)
    today_time = (datetime.now().time()).strftime("%H:%M")
    today_vehicles = Vehicle.query.filter_by(VeEntryDate=today_date).all()
    types = VehicleTypes.query.all()
    departments = Department.query.all()
    return render_template('Dataentry/dataentry-vehicle.html',departments=departments, today_vehicles=today_vehicles,today_date=today_date,today_time=today_time, types=types)
...
def post_mill():
    today_date = ''
    if "saved_date_mill" in session:
        today_date = session['saved_date_mill']
        today_date = session_date(today_date)
    today_time = (datetime.now().time()).strftime("%H:%M")
    comp_vehicles = CompanyVehicle.query.all()  
    comp_vehicles_today = CompanyTimesheet.query.filter_by(date=today_date).all()
    departments = Department.query.all()
    return render_template('Dataentry/dataentry-mill.html',departments=departments, today_date=today_date, today_time=today_time, comp_vehicles=comp_vehicles, comp_vehicles_today=comp_vehicles_today)
   ...
   if request.form:
       ...
       if Visitor.query.filter_by(contact=contact).first():
           ...
           #Adds new Visitor if it doesnt exist
            visitor = Visitor(name=name, contact=contact, place_from=place_from, plant_id=3)
       ...
       timelog = Timesheet_Visitor(visitor_id=visitor.id, activity_id=activity.id, extras=extras , date=date, in_time=time, plant_id=3)
